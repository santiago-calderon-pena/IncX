    ...
    def explain_frame(self, image) -> tuple[Dict[int, IncRexOutput], np.ndarray]:
        ...
        if self._frame_number == 0:
            ...
            if min(self._object_indices) < 0:
                raise ValueError(
                    "One of the provided values in object_indexes is less than zero."
                )
            ...
            for object_index in self._object_indices:
                ...
                bounding_boxes_dict[object_index] = prediction.bounding_boxes[
                    object_index
                ]

            self._explanation_tracker = ExpTracker(
                saliency_maps_dict, bounding_boxes_dict, prediction
            )

            for object_index in self._object_indices:
                bounding_box = prediction.bounding_boxes[object_index]
                self._obj_classes_ix[object_index] = np.argmax(
                    prediction.class_scores[object_index]
                )
                sufficient_explanation, exp_threshold, mask = (
                    compute_initial_sufficient_explanation(
                        self._model,
                        saliency_maps[object_index],
                        image,
                        self._obj_classes_ix[object_index],
                        bounding_box,
                        divisions=self._saliency_map_divisions,
                        minimum=np.min(saliency_maps[object_index]),
                        maximum=np.max(saliency_maps[object_index]),
                    )
                )
                ...
                results[object_index] = IncRexOutput(
                    saliency_map=saliency_maps[object_index],
                    bounding_box=bounding_box,
                    sufficient_explanation=sufficient_explanation,
                    label=coco_labels[self._obj_classes_ix[object_index]],
                    score=float(max(prediction.class_scores[object_index])),
                    mask=mask,
                ...
                if score > 0:
                    sufficient_explanation, exp_threshold, mask = (
                        compute_initial_sufficient_explanation(
                            ...
                            bounding_box,
                            divisions=5,
                            minimum=self._exp_thresholds[object_index] * 0.8,
                            maximum=self._exp_thresholds[object_index] * 1.2,
                        )
                    )
                    self._exp_thresholds[object_index] = exp_threshold
        ...
        for object_index, el in results.items():
            ...
            frame = cv2.rectangle(
                frame,
                (int(el.bounding_box[0]), int(el.bounding_box[1])),
                (int(el.bounding_box[2]), int(el.bounding_box[3])),
                bright_red,
                thickness=3,
            )
            el.sufficient_explanation = cv2.rectangle(
