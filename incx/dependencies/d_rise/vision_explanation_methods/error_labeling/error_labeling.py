...
class ErrorLabeling:
    ...
    def __init__(
        self, task_type: str, pred_y: list, true_y: list, iou_threshold: float = 0.5
    ):
        """
    ...
    def compute_error_labels(self):
        ...
        :rtype: NDArray
        """
        match_matrix = np.full((len(self._true_y), len(self._pred_y)), None)
        # save original ordering of predictions
        original_indices = [
            i
            for i, _ in sorted(
                enumerate(self._pred_y), key=lambda x: x[1][-1], reverse=True
            )
        ]
        ...
        if len(self._true_y) == 0:
            match_matrix = np.array(
                [[ErrorLabelType.BACKGROUND] for _ in range(len(self._pred_y))]
            )
            return match_matrix
        ...
        for gt_index, gt in enumerate(self._true_y):
            for detect_index, detect in enumerate(sorted_list):
                ...
                if iou_score.item() == 0:
                    ...
                    match_matrix[gt_index][detect_index] = ErrorLabelType.BACKGROUND
                    ...
                    if detect[0] != gt[0]:
                        ...
                        match_matrix[gt_index][detect_index] = ErrorLabelType.CLASS_NAME
                    elif ErrorLabelType.MATCH in match_matrix[gt_index]:
                        ...
                        match_matrix[gt_index][detect_index] = ErrorLabelType.MATCH
                else:
                    if detect[0] != gt[0]:
        ...
        # scores)
        match_matrix[gt_index] = [match_matrix[gt_index][i] for i in original_indices]
        return match_matrix
    ...
    def compute_error_list(self):
        ...
        dup_count = np.count_nonzero(match_matrix == ErrorLabelType.DUPLICATE_DETECTION)
        error_list = [ErrorLabelType.DUPLICATE_DETECTION for _ in range(dup_count)]
            ...
            for gt_index, gt in enumerate(error_arr):
                for detect_index, detect in enumerate(gt):
                    if detect == err:
                        ...
                        error_arr = self._remove_rows_cols(
                            error_arr, set([gt_index]), set([detect_index])
                        )
                        if len(error_arr) == 0:
                            break
    ...
    def _remove_matches(self, arr: np.array):
        ...
        modified_array = self._remove_rows_cols(arr, rows_to_delete, cols_to_delete)

        return modified_array

    def _remove_rows_cols(
        self, arr: np.array, rows_to_delete: set, cols_to_delete: set
    ):
        """
        ...
        # Delete rows
        modified_array = [
            row for row_index, row in enumerate(arr) if row_index not in rows_to_delete
        ]
