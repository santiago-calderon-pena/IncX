...
container_client_incx = blob_service_client.get_container_client(container=AZURE_CONTAINER_NAME)
...
container_client_drise = blob_service_client.get_container_client(container=AZURE_CONTAINER_NAME)
...
while blob_names_incx:
    ...
    deletion_drise = compute_deletion(model=model, saliency_map=saliency_map_drise, image = image,
                                      class_index=class_index, bounding_box=bounding_box, object_index=current_index
                                      , divisions=100)
    deletion_incx = compute_deletion(model=model, saliency_map=saliency_map_incx, image = image,
                                        class_index=class_index, bounding_box=bounding_box, object_index=current_index
                                        , divisions=100)
    
    insertion_drise = compute_insertion(model=model, saliency_map=saliency_map_drise, image = image,
                                        class_index=class_index, bounding_box=bounding_box, object_index=current_index
                                        , divisions=100)
    
    insertion_incx = compute_insertion(model=model, saliency_map=saliency_map_incx, image = image,
                                        class_index=class_index, bounding_box=bounding_box, object_index=current_index
                                        , divisions=100)
    
    epg_drise = compute_energy_based_pointing_game(saliency_map=saliency_map_drise, bounding_box=bounding_box)
    
    epg_incx = compute_energy_based_pointing_game(saliency_map=saliency_map_incx, bounding_box=bounding_box)
    ...
    with lock_comparison:
        ...
        metrics_results['D-RISE'][model_name]['Insertion'][image_number][frame_number] = insertion_drise
        metrics_results['D-RISE'][model_name]['Deletion'][image_number][frame_number] = deletion_drise
        metrics_results['D-RISE'][model_name]['EPG'][image_number][frame_number] = epg_drise
        metrics_results['D-RISE'][model_name]['Explanation Proportion'][image_number][frame_number] = exp_proportion_drise
        metrics_results['D-RISE'][model_name]['Time'][image_number][frame_number] = time_drise
        
        metrics_results['Incx'][model_name]['Insertion'][image_number][frame_number] = insertion_incx
        metrics_results['Incx'][model_name]['Deletion'][image_number][frame_number] = deletion_incx
        metrics_results['Incx'][model_name]['EPG'][image_number][frame_number] = epg_incx
        metrics_results['Incx'][model_name]['Explanation Proportion'][image_number][frame_number] = exp_proportion_incx
        metrics_results['Incx'][model_name]['Time'][image_number][frame_number] = time_incx
