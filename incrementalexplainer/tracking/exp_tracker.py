...
class ExpTracker:
    def __init__(
        self,
        initial_saliency_maps: dict,
        initial_bounding_boxes: dict,
        initial_prediction: od_common.DetectionRecord,
    ):
        if len(initial_saliency_maps) != len(initial_bounding_boxes):
            raise ValueError(
                "The given saliency maps and boxes do not have the same length"
            )
        ...
        detections = [
            ...
            for i, (bb, score) in enumerate(
                zip(initial_prediction.bounding_boxes, initial_prediction.class_scores)
            )
        ]
        ...
        for object_index, bounding_box in self._initial_bounding_boxes.items():
            self._object_to_ids[object_index] = bboxes_to_id[
                (
                    int(bounding_box[0]),
                    int(bounding_box[1]),
                    int(bounding_box[2]),
    ...
    def compute_tracked_explanation(self, image, prediction: od_common.DetectionRecord):
        detections = [
            ...
            for i, (bb, score) in enumerate(
                zip(prediction.bounding_boxes, prediction.class_scores)
            )
        ]
        results = {}
        if not detections:
            for object_index in self._object_to_ids.keys():
                results[object_index] = (
                    np.zeros_like(self._initial_saliency_maps[object_index]),
                    (0, 0, 0, 0),
                    0,
                    -1,
                )
        ...
        for object_index, id in self._object_to_ids.items():
            matching_result = next(
                (result for result in result_tracker if result[4] == id), np.array([])
            )
            ...
            x1_old, y1_old, x2_old, y2_old = (
                self._initial_bounding_boxes[object_index].cpu().numpy().astype(int)
            )
            ...
            scaled_saliency_map = scale_image(
                self._initial_saliency_maps[object_index], scale_x, scale_y
            )
            ...
            scaled_map_center = np.array(
                [scaled_saliency_map.shape[1] / 2, scaled_saliency_map.shape[0] / 2]
            )

            scaled_bb_center = np.array(
                [
                    (old_bb_center[0] - image_center[0]) * scale_x
                    + scaled_map_center[0],
                    (old_bb_center[1] - image_center[1]) * scale_y
                    + scaled_map_center[1],
                ]
            )
