...
def test_pytorch_and_cuda_versions():
    ...
    if pytorch_response.status_code != 200:
        raise AssertionError(f"PyTorch package '{pytorch_package}' does not exist on PyPI.")
    ...
    # Then
    assert compare_versions(pytorch_version, pytorch_min_version) >= 0, \
    ...
    assert compare_versions(installed_cuda_version, cuda_min_version) >= 0, \
